...
export const stripeApi = {
  ...
  createPaymentIntent: async (
    ...
    const response = await apiService.post('/stripe/create_intent', {
      amount,
      currency,
    });
  ...
  getPaymentIntent: async (id: string): Promise<PaymentIntentDetailsResponse> => {
    const response = await apiService.get(`/stripe/payment_intent/${id}`);
    return response.data as PaymentIntentDetailsResponse;
  },
  ...
  confirmPaymentIntent: async (
    ...
    const response = await apiService.post('/stripe/confirm_intent', {
